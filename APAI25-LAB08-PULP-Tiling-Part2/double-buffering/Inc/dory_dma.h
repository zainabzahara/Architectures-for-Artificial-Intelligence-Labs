...
static void dory_dma_init() {
  tid = mchan_transfer_get_id();
}

static void dory_dma_term() {
  mchan_transfer_free(tid);
}

static void dory_dma_wait() {
  mchan_transfer_wait(tid);
}

static void dory_dma_memcpy_1d_async(dory_dma_copy *copy) {
...
static void dory_dma_memcpy_2d_async(dory_dma_copy *copy) {
...
static void dory_dma_memcpy_3d_async(dory_dma_copy *copy) {
  void *ext = copy->ext;
  void *loc = copy->loc;
  const int size_2d = copy->number_of_1d_copies * copy->length_1d_copy;
...
static void dory_dma_memcpy_async(dory_dma_copy *copy) {
