                  ...
                  uint16_t stride_y)
{
  ...
  uint8_t extra_chunk = ((dim_out_y & (NB_CORES-1)) != 0);
  ...
  {
    Log2Core = log2(NB_CORES >> 1);
    core_id_r = (core_id >> 1);
    dim_out_x_r = (dim_out_x >> 1);
    section = (core_id & 0x1);
    extra_chunk_r = ((dim_out_y & ((NB_CORES >> 1) - 1)) != 0);
  }
  else
  {
    Log2Core = log2(NB_CORES);
    core_id_r = core_id;
