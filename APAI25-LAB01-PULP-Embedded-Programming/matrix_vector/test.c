...
void stop_perf_counter()
{
	...
	*/
	...
	float cpi =  (float)cycles_counter / (float)instr_counter;/* YOUR CODE HERE*/
	// MAC/Cycles
	float mac_on_cycles = (float)mac_counter / (float)cycles_counter;/* YOUR CODE HERE*/
	// Instructions/Cycles
	float instructions_on_cycles =(float)instr_counter / (float)cycles_counter; /* YOUR CODE HERE*/
	// Instructions/MAC
	float instructions_on_mac =  (float)instr_counter / (float)mac_counter;
/* YOUR CODE HERE*/
	...
	*/
	...
	for (int i = 0; i < size_N; i++) // outer looop of the gemv
	{
		for (int j = 0; j < size_M; j++) // inner loop of the gemv
		{
			...
			vec_o[i] += mat_i[i * size_M + j] * vec_i[j]; //MAC operation
...
int main()
{
	...
	/* call the GEneric Matrix-Vector (gemv) function with profiling */
    start_perf_counter();
    gemv(N, M, matrix, vector, output_vec);
    stop_perf_counter();
